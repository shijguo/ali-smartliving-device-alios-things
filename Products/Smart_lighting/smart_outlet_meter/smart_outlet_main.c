...
#define CALC_METER_DATA_PERIOD_S       (60)
#define REPORT_CURRENT_METER_PERIOD_S  (60)
#define REPORT_CONSUMPTION_PERIOD_S    (900)

#define KV_KEY_CONSUMPTION "OUTLET_CONSUMPTION"

const char *report_prop_list[4] = {"Current", "CurrentVoltage", "RealTimePower", "UnitPowerConsumption"};
...
static void user_deviceinfo_update(void)
{
    int res = 0;
    user_example_ctx_t *user_example_ctx = user_example_get_ctx();
    char *device_info_update = "[{\"attrKey\":\"OutletFWVersion\",\"attrValue\":\"1.6.0\"}]";
        ...
        int *response_len)
{
    ...
    for (int index = 0; index < cJSON_GetArraySize(request_root); index++) {
        ...
        } else if (strcmp("Current", item_propertyid->valuestring) == 0) {
            cJSON_AddNumberToObject(response_root, "Current", device_status->current);
        } else if (strcmp("CurrentVoltage", item_propertyid->valuestring) == 0) {
            cJSON_AddNumberToObject(response_root, "CurrentVoltage", device_status->current_voltage);
        } else if (strcmp("RealTimePower", item_propertyid->valuestring) == 0) {
            cJSON_AddNumberToObject(response_root, "RealTimePower", device_status->realtime_power);
#ifdef AOS_TIMER_SERVICE
...
void user_save_consumption(void)
{
    user_example_ctx_t *user_example_ctx = user_example_get_ctx();
	device_status_t *device_status = &user_example_ctx->status;
    float consumption = 0.0;

    consumption = device_status->unit_consumption;
    aos_kv_set(KV_KEY_CONSUMPTION, (const void *)&consumption, sizeof(float), 1);
}

void user_calc_meter_data(void)
{
    int res = 0;
    user_example_ctx_t *user_example_ctx = user_example_get_ctx();
	device_status_t *device_status = &user_example_ctx->status;
    ...
    double realtime_power = 0.0;
    float consumption = 0.0;

#if defined(ENERGY_METER_ENABLE)
    meter_data_t meter_data;
    memset(&meter_data, 0, sizeof(meter_data_t));
    res = get_meter_data(&meter_data);
    if (res != 0) {
        LOG_TRACE("get meter data failed, res:%d", res);
    } else {
        current = (double)meter_data.current;
        current_voltage = (double)meter_data.voltage;
        realtime_power = (double)meter_data.power;
        device_status->current = current;
        device_status->current_voltage = current_voltage;
        device_status->realtime_power = realtime_power;
        LOG_TRACE("current:%lf, current_voltage:%lf, realtime_power:%lf", current, current_voltage, realtime_power);

        consumption = (float)((float)realtime_power * CALC_METER_DATA_PERIOD_S) / (float)(3600 * 1000);
        device_status->unit_consumption = device_status->unit_consumption + consumption;
        LOG_TRACE("cur_consumption:%f, unit_consumption:%f", consumption, device_status->unit_consumption);
    }
#endif
}

void user_post_current_meter(void)
{
    int res = 0;
    user_example_ctx_t *user_example_ctx = user_example_get_ctx();
	device_status_t *device_status = &user_example_ctx->status;
    ...
    current = device_status->current;
    current_voltage = device_status->current_voltage;
    realtime_power = device_status->realtime_power;

    snprintf(property_payload, sizeof(property_payload), "{\"%s\":%.2lf,\"%s\":%.2lf,\"%s\":%.2lf}",
            report_prop_list[0], current, report_prop_list[1], current_voltage, report_prop_list[2], realtime_power);

    res = IOT_Linkkit_Report(user_example_ctx->master_devid, ITM_MSG_POST_PROPERTY,
            property_payload, strlen(property_payload));

    LOG_TRACE("Post Current Meter Message ID: %d payload %s", res, property_payload);
}

void user_post_consumption(void)
{
    int res = 0;
    user_example_ctx_t *user_example_ctx = user_example_get_ctx();
	device_status_t *device_status = &user_example_ctx->status;
    char property_payload[64];
    float unit_consumption = 0.0;

    unit_consumption = device_status->unit_consumption;

    snprintf(property_payload, sizeof(property_payload), "{\"%s\":%f}",
            report_prop_list[3], unit_consumption);

    res = IOT_Linkkit_Report(user_example_ctx->master_devid, ITM_MSG_POST_PROPERTY,
            property_payload, strlen(property_payload));
    if (res > 0) {
        device_status->unit_consumption = 0.0;
    }

    LOG_TRACE("Post UnitPowerConsumption Message ID: %d payload %s", res, property_payload);
}
...
void user_post_property_after_connected(void)
{
    ...
    snprintf(property_payload, sizeof(property_payload), "{\"PowerSwitch\":%d, \"Current\":%f, \"CurrentVoltage\":%f, \"RealTimePower\":%f}", \
            device_status->powerswitch, device_status->current, \
            device_status->current_voltage, device_status->realtime_power);
    res = IOT_Linkkit_Report(user_example_ctx->master_devid, ITM_MSG_POST_PROPERTY,
            property_payload, strlen(property_payload));
...
int linkkit_main(void *paras)
{
    ...
    aos_kv_get(KV_KEY_CONSUMPTION, (void *)&consumption, &len);
    LOG_TRACE("get save consumption:%f\n", consumption);
    device_status->unit_consumption = consumption;
...
#if defined(ENERGY_METER_ENABLE)
        /* calculation meter data */
        if ((time_now_sec % CALC_METER_DATA_PERIOD_S == 0) || (time_now_sec - time_begin_sec == 10)) {
            user_calc_meter_data();
        }

        /* post current meter */
        if ((time_now_sec % REPORT_CURRENT_METER_PERIOD_S == 0) || (time_now_sec - time_begin_sec == 10)) {
            if (user_master_dev_available()) {
                user_post_current_meter();
            }
        }

        /* post unit power consumption */
        if ((time_now_sec % REPORT_CONSUMPTION_PERIOD_S == 0) || (time_prev_sec == 0)) {
            if (user_master_dev_available()) {
                user_post_consumption();
            }
            if (time_now_sec % REPORT_CONSUMPTION_PERIOD_S == 0) {
                user_save_consumption();
            }
        }
#endif
