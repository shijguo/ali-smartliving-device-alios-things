...
#Firmware image path
project_example=smart_outlet_meter
project_board=uno-91h
product_vendor=ali
product_name=meter_outlet
...
aos_cube=aos
aos_path=${PWD}/../../sdk_aos/Living_SDK

# files to make static library
src_files_for_lib=hlw8012.c
# static library name
static_library_name=meter

# cross toolchain
cross_toolchain_prefix=

# arch header files path
arch_header_files_path=
# cflags
arch_cflags=
# target arch
target_arch="unknown"
...
{
	local target_arch_string=`grep "HOST_ARCH" ${aos_path}/board/${project_board}/${project_board}.mk`
	target_arch_string="${target_arch_string##*:=}"
	target_arch=`echo $target_arch_string | tr '\r' '\0'`
}

function get_cross_prefix_by_arch()
{
	if [ "$target_arch" == "Cortex-M4" ]; then
		cross_toolchain_prefix=arm-none-eabi-
	elif [ "$target_arch" == "ARM968E-S" ]; then
		cross_toolchain_prefix=arm-none-eabi-
	fi
}

function get_arch_header_files_path()
{
	if [ "$target_arch" == "Cortex-M4" ]; then
		arch_header_files_path=${aos_path}/platform/arch/arm/armv7m/gcc/m4
	elif [ "$target_arch" == "ARM968E-S" ]; then
		arch_header_files_path=${aos_path}/platform/arch/arm/armv5/gcc/
	fi
}

function get_arch_cflags()
{
	if [ "$target_arch" == "Cortex-M4" ]; then
		arch_cflags="-mtune=cortex-m4 -march=armv7-m -mfloat-abi=soft -mthumb -mthumb-interwork -mlittle-endian"
	elif [ "$target_arch" == "ARM968E-S" ]; then
		arch_cflags="-mtune=arm968e-s -march=armv5te -mfloat-abi=soft -mthumb -mthumb-interwork -mlittle-endian"
	fi
}
...
function make_static_library()
{
	rm -rf lib
	mkdir lib
	mkdir lib/${target_arch}
	get_target_arch
	get_cross_prefix_by_arch
	get_arch_header_files_path
	get_arch_cflags
	${cross_toolchain_prefix}gcc -c ${src_files_for_lib} -DCONFIG_NO_TCPIP ${arch_cflags} -I . -I ${aos_path}/include -I ${arch_header_files_path} -o ${static_library_name}.o
        ${cross_toolchain_prefix}ar rcs lib/${target_arch}/lib${static_library_name}.a ${static_library_name}.o
}
...
@@@ MCU CORE:      ${target_arch}
@@@ Application:   ${project_example}
*************************************************************************

(0) gen static library for ${static_library_name}
...
EOF
	...
	case $input in
	0) echo "gen static library for ${static_library_name}"
	make_static_library
	exit;;
